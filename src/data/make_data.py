...
def make_data(sample):
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        file = f'data_{sample}.csv'
        filepath = os.path.join(config.TEMPDIR, file)
        print('Splitting data...')
        split_file(filepath, tempdir)
        ...
        for piece in raw_pieces:
            ...
            clean_piece = (
                read_raw(piece)
                .pipe(clean_data)
                .pipe(select_sample_hacioglu)
                # .pipe(clean_data)
            )
            clean_pieces.append(clean_piece)
        clean_name = f'data_{sample}_hacioglu.parquet'
